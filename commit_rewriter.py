...
class Repository:
    ...
    def commits(self):
        ...
        for oid in self.git("log", "-100", "--format=%H", self.ref).decode().splitlines():
            ...
            commits.append({"oid": oid, "message": decoded, "author": author.decode(errors="replace"), "committer": committer.decode(errors="replace")})
        return commits
...
function render(){ $('commits').replaceChildren();for(const c of state.commits){const card=el('article');card.dataset.oid=c.oid;card.id='commit-'+c.oid;card.classList.toggle('edited',c.oid in drafts);card.append(el('div',`${c.oid.slice(0,12)} · ${c.author}`,'meta'));const details=el('details');details.append(el('summary','Commit metadata'),el('pre',`Author: ${c.author}\nCommitter: ${c.committer}`));card.append(details);const area=el('textarea');area.value=drafts[c.oid]??c.message;area.disabled=busy||stale;area.setAttribute('aria-label',`Message for commit ${c.oid.slice(0,12)}`);area.spellcheck=false;const note=el('div',undefined,'validation');note.id=`v-${c.oid}`;area.setAttribute('aria-describedby',note.id);validation(area,note);area.oninput=()=>{if(area.value===c.message)delete drafts[c.oid];else drafts[c.oid]=area.value;card.classList.toggle('edited',c.oid in drafts);validation(area,note);save();counts();filter()};card.append(area,note);$('commits').append(card)}counts();filter()}
